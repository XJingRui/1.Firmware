...
void StartupView::Create(lv_obj_t *root)
{
    ...
    lv_label_set_text(label, "X-Knob");
    lv_obj_center(label);
    ui.labelLogo = label;
...
/*
对lvgl的组件进一步进行封装 
执行时间 执行对象 （x y 高度 宽度） 起始值  结束值  
*/
    lv_anim_timeline_wrapper_t wrapper[] =
        {
            ANIM_DEF(0, ui.cont, height, 0, lv_obj_get_style_height(ui.cont, 0)),
    ...
    // 将定义的动画加入到at（）动画化时间线 中进行管理 后面执行动画直接调用就行 
}

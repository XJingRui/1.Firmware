...
void Page::PlaygroundView::SetPlaygroundMode(int16_t mode)
{
	...
	switch (app) {
		...
		case PLAYGROUND_MODE_BOUND:
			MIN_VALUE = 0;
			MAX_VALUE = 12;
			SCALE_LEFT_BOUND_TICKS = 160;
			SCALE_ANGLE_RANGE = 220;
			BoundZeroView();
			break;
...
void PlaygroundView::Create(lv_obj_t* root)
{
	...
	lv_meter_set_scale_ticks(ui.meter, ui.scale_pot, 41, 2, 0, lv_color_make(0xff, 0xcc, 0xcc));
	...
	lv_meter_set_scale_ticks(ui.meter, ui.scale_arc, 41, 0, 0, lv_color_make(0xff, 0xcc, 0xcc));
	lv_meter_set_scale_range(ui.meter, ui.scale_arc, 0, 100, 360, ARC_START_ROTATION);

	//add arc for ui.scale_value添加指示弧和设置其属性：
	ui.arc = lv_meter_add_arc(ui.meter, ui.scale_arc, 6, lv_color_make(0xff, 0xcc, 0xcc), 5);
	lv_meter_set_indicator_start_value(ui.meter, ui.arc, 0);
	lv_meter_set_indicator_end_value(ui.meter, ui.arc, 0);

	//Write style state: LV_STATE_DEFAULT for meter_style设置仪表的样式：
	lv_style_init(&style.meter);
	lv_style_set_radius(&style.meter, 240);
	lv_style_set_bg_color(&style.meter, lv_color_black());
	lv_style_set_bg_grad_color(&style.meter, lv_color_make(0xff,0xff,0xcc));//  仪表盘颜色
	...
	//Write style state: LV_STATE_DEFAULT for style_screen_meter_1_main_ticks_default设置刻度文本的样式：
	...
	//Write codes screen_label_1  创建标签对象
	...
	//Write style state: LV_STATE_DEFAULT for label_style设置标签样式：
	static lv_style_t label_style;
	lv_style_init(&label_style);
	lv_style_set_radius(&label_style, 0);
	lv_style_set_bg_color(&label_style, lv_color_make(0xff, 0xff, 0xaa));
	lv_style_set_bg_grad_color(&label_style, lv_color_make(0xcc, 0xff, 0xff));
	lv_style_set_bg_grad_dir(&label_style, LV_GRAD_DIR_NONE);
	lv_style_set_bg_opa(&label_style, 0);  // 完全透明
	lv_style_set_text_color(&label_style, lv_color_make(0xcc, 0xff, 0xff));   

...
void ffat_init() {
    ...
    if (get_init_ffat()) {
// 尝试挂载文件系统到 `/LFS` 路径。如果挂载失败（`begin` 函数返回 `false`），则通过串行端口输出错误信息并退出函数。
    ...
    // 打开文件系统的根目录，并尝试打开第一个文件。
    File root = FILESYSTEM.open("/");
    File file = root.openNextFile();
    // 使用循环遍历根目录下的所有文件。对于每个文件，获取文件名和大小，并通过串行端口输出。
    while (file) {
...
void wifi_server_begin(void *parameter) {
    ...
    MDNS.begin(host);
    printf("Open http://%s.local to see the file browser\n", host);
